	...
	for (k = 0; k < Scena.size(); k++) {
		...
		*/
		if (k == 0) {
			...
			Scena[k].Model = translate(Scena[k].Model, vec3(600.0f, 200.0f, 0.0f));
			Scena[k].Model = scale(Scena[k].Model, vec3(100.0f, 100.0f, 1.0f));
		}
		else {
			Scena[k].Model = mat4(1.0f);
			Scena[k].Model = translate(Scena[k].Model, vec3(400.0f, 400.0f, 0.0f));
			Scena[k].Model = scale(Scena[k].Model, vec3(100.0f, 100.0f, 1.0f));
		}
...
int main(int argc, char* argv[]) {
	...
	glutCreateWindow("Normalizzato OpenGL");
