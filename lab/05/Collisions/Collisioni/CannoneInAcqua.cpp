...
/*
	Bounding box: si calcolano il minimo e massimo sia per x che per y e si cerca di capire se in un rettangolo
	virtuale due oggetti si compenentrano e quindi collidono.
	Il controllo si riduce a controllare la collisione sia in x che in y e se i due oggetti si sovrappongono in 
	entrambe le dimensioni allora si ha una collisione.
*/
...
void drawScene(void) {
	...
	// Modifica mia: la farfalla sparisce e poi riappare
	 if (Coll == FALSE) {
