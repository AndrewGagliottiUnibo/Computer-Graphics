...
void crea_piano(Mesh* mesh, vec4 color)
{
	...
	// Se esco da uno, la texture verrà replicata molteplici volte
	// Questo fenomeno è chiamato wrapping della texture: cioè come 
	// vengono gestiti al di fuori della copertura totale 1.0 della
	// zona da dove texturizzare
	...
	// Mappo in modi diverso la texture
...
// La costruzione è una unione di circonferenza in cui si uniscono anelli adiecenti con collegamenti
// esatti affinché possano essere rappresentati correttamente
void crea_sfera(Mesh* mesh, vec4 colore)
{
	...
	// Calc The Vertices
	// Stacks = circonferenze lungo l'asse y
	for (int i = 0; i <= Stacks; ++i) {
		...
		for (int j = 0; j <= Slices; ++j) {
			...
			// Coordinata di Texture, cioè i parametri della sfera
			s = U;
			t = V;
