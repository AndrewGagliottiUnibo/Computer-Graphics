...
void drawScene(void)
{
	...
	Disegno Sfondo
	*/
	// Lo sfondo non viene disegnato sullo z-buffer (si disattiva l'aggiornamento) ed è disegnato per primo
	// il cubo quindi rimarrà lì piccolino ma sarà poi visibile come tutta la finestra. Ci deve essere sempre.
   ...
   //all'interno del Vertex shader. Uso l'identificatio MatView
	...
	for (int k = 2; k < Scena.size(); k++)
	{
		...
		// Avendo costruito entrambe le texture, dobbiamo decidere quale utilizzare.
		// Si fa il binding solo di cosa ci serve: fissata l'unità di elaborazione, che è
		// sempre la stessa, gli passo un oggetto texture diverso su cui lavorare.
		if (k < Scena.size() - 1)
		{
			// Unità di texture unica in cui cambio oggetto a seconda di cosa devo disegnare: si può
			// operare anche diversamente facendo sì che per un oggetto texture è sempre associato ad 
			// un'unità di texture. Inoltre, questo procedimento vale anche nel caso in cui si voglia
			// adottare il multitexturing (diverse texture per diverse unità di texturing) su uno 
			// stesso oggetto grafico.
		...
		{
			// Stessa unità ma oggetto texture diverso
			glUniform1i(loc_texture, 0);
			glBindTexture(GL_TEXTURE_2D, texture1);
		}
