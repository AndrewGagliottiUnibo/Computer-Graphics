...
// .obj sono file di testo che contengono numerosi dati per modellare degli oggetti
// I vertici saranno inseriti nel VAO per poi essere rielaborati come sempre.
// Per comprendere poi come gli oggetti si comportino nella scena, bisogna che si guardi i file.mtl
bool loadAssImp(const char* path, vector<Mesh> & mymesh)
 {

	Assimp::Importer importer;

	// Triangulate attiva automaticamente la trinagolazione delle figure, senza doverlo fare noi manualmente.
	...
	for (unsigned int nm=0; nm<num_meshes;nm++)
	{
		...
		// In color si salvano i colori per ciascuna geometria
