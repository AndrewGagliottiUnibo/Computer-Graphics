...
void mykeyboard(unsigned char key, int x, int y)
{
	...
	{
		// valore per il quale si diegna il grafico, e' il nostro intervallo
		float tg;
		int j;
		Poligonale.CP = Curva.CP;
		Poligonale.colCP = Curva.colCP;
		...
		if (Poligonale.CP.size() > 1)
		{
			...
			for (tg = 0; tg <= 1; tg += passotg)
			{
				...
				// facio il push back dei vertici della curva
	...
	void drawScene(void)
	{
		...
		//Caso metodo=2 : costrusci curva di bezier
		if (metodo == 2)
		{
			costruisci_formaBezier(col_top, col_bottom, &Curva);
			crea_VAO_Vector(&Curva);
			crea_VAO_CP(&Poligonale);
